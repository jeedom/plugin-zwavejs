...
$("body").off("click", ".addAssociation").on("click", ".addAssociation", function(e) {
	...
	if ($(".selectTargetNode option:selected").value() == '') {
		$('#div_nodeAddGroupsZwaveJsAlert').showAlert({ message: '{{Vous devez choisir un nœud cible pour ajouter une association}}', level: 'danger' })
		return
	}
		...
		success: function(NodeAssociations) {
			if (NodeAssociations['id'] == nodeId) {
				if (nodes[nodeId]['status'] == 'Dead') {
					$('#div_StatusGroupAlert').empty().append('<div class="alert alert-warning" role="alert">{{Le nœud est en statut "présumé mort", il n\'y a donc pas de groupes à afficher.}}</div>')
				}
				var body = ''
				var bodylist = ''
...
$('#div_nodeGroupsZwaveJsAlert').showAlert({ message: '{{Chargement des informations en cours}}...', level: 'warning' })
