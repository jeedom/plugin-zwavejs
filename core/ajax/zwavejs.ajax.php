...
try {
	...
	if (init('action') == 'uploadOTA') {
		...
		if (!file_exists($uploaddir . '/' . $_FILES['file']['name'])) {
			...
			zwavejs::publishMqttApi('updateFirmware', array("args"=>array(intval(init("node")),array(array('name'=>$_FILES['file']['name'],'data'=>array("type"=>"Buffer","data"=>$byteArr),'target'=>0)))));
		}
		ajax::success();
	}
	
	if (init('action') == 'uploadOTW') {
		$uploaddir = dirname(__FILE__). '/../../tmp';
		if (!file_exists($uploaddir)) {
			mkdir($uploaddir);
		}
		...
		if (!in_array($extension, array('.bin','.hex','.ex_','.gbl'))) {
			throw new Exception('Extension du fichier non valide (autorisé .bin, .hex, .ex_, .gbl) : ' . $extension);
		}
		if (filesize($_FILES['file']['tmp_name']) > 2000000) {
			throw new Exception(__('Le fichier est trop gros (maximum 2mo)', __FILE__));
		}
		...
		if (!file_exists($uploaddir . '/' . $_FILES['file']['name'])) {
			...
			zwavejs::publishMqttApi('firmwareUpdateOTW', array("args"=>array(array('name'=>$_FILES['file']['name'],'data'=>array("type"=>"Buffer","data"=>$byteArr)))));
		}
		ajax::success();
	}
